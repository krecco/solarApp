<template>
  <div>
    <b-link
      :to="{ name: 'admin-settings-list-campaign' }"
      variant="success"
...
export default {
  ...
  setup() {
    ...
    const fetchSettings = () => {
      store
        .dispatch(`${STORE_MODULE_NAME}/fetchSettingsCampaign`, { settingsId: router.currentRoute.params.id })
        .then(response => {
    ...
    const onSubmitSettingsData = () => {
      store.dispatch(`${STORE_MODULE_NAME}/editSettingsCampaign`, { baseData: settingsData.value })
        .then(() => {
          toast({
