  ...
  <div>
    ...
    >
      <b-row>
        ...
        >
          ...
          >
            <b-col
              cols="10"
            >
              <h5>Eigentümer der Liegenschaft</h5>
            </b-col>
            <b-col
              cols="2"
            >
              <b-form-checkbox
                id="propertyOwnerListFinished"
                v-model="baseData.propertyOwnerListFinished"
                style="float:right;"
                switch
                inline
                :disabled="isDisabled()"
                @change="propertyOwnerListFinished"
              >
                Eigentümer erledigt
              </b-form-checkbox>
            </b-col>
        ...
        <b-col
          v-if="baseData.orderInterest === true"
          cols="12"
        ...
        >
          <b-form-checkbox
            ...
            v-model="baseData.contractsReviewed"
            style="float:right;"
            switch
            inline
            :disabled="isDisabled()"
            @change="updateContractsReviewedStatus"
          >
            Vertragsdaten erledigt
          </b-form-checkbox>
          <b-form-checkbox
            v-if="canFinishSegment !== true"
            style="float:right;"
        ...
        >
          <b-form-checkbox
            id="contractsSigned"
            v-model="baseData.contractsSigned"
            style="float:right;"
            switch
            inline
            :disabled="isDisabled()"
            @change="updateContractsSignedStatus"
          >
            Vertragsunterlagen überprüft
          </b-form-checkbox>
        </b-col>
        ...
        >
                  ...
                  >
                    ...
                    >
                      <b-button
                        ...
                        type="button"
                        @click="generateEnergySavingContract($router.currentRoute.params.id, User.id, row)"
                      >
                        <feather-icon
                          icon="CpuIcon"
                          class="mr-50"
                        />
                        Vertrag Energieeinsparung neu generieren
                      </b-button>
                    </div>
                    <div
                      v-if="row.item.type === 25"
                      style="text-align:right; padding-bottom:10px;"
                    >
                      <b-button
                        v-if="isDisabled() == false"
                        ...
                        :block="$store.getters['app/currentBreakPoint'] === 'xs'"
                        type="button"
                        @click="generateContractBillingSheet($router.currentRoute.params.id, User.id, row)"
                      >
                        <feather-icon
                          icon="CpuIcon"
                          class="mr-50"
                        />
                        Vertrag Verrechnungsblatt neu generieren
                      </b-button>
                    </div>
                    <div
                      v-if="row.item.type === 27"
                      style="text-align:right; padding-bottom:10px;"
                    >
                      <b-button
                        v-if="isDisabled() == false"
                        ...
                        :block="$store.getters['app/currentBreakPoint'] === 'xs'"
                        type="button"
                        @click="generateMandateCompletion($router.currentRoute.params.id, row)"
                      >
                        <feather-icon
                          icon="CpuIcon"
                          class="mr-50"
                    ...
                    >
                      <b-button
                        ...
                        :block="$store.getters['app/currentBreakPoint'] === 'xs'"
                        type="button"
                        @click="generateMandateBilling($router.currentRoute.params.id, row)"
                      >
                        <feather-icon
                          icon="CpuIcon"
                          class="mr-50"
                        />
                        Vollmacht Energieabrechnung neu generieren
                      </b-button>
                    </div>
                    <div
                      v-if="row.item.type === 291"
                      style="text-align:right; padding-bottom:10px;"
                    >
                      <b-button
                        v-if="isDisabled() == false"
                        ...
                        :block="$store.getters['app/currentBreakPoint'] === 'xs'"
                        type="button"
                        @click="generateMandateBillingNet($router.currentRoute.params.id, row)"
                      >
                        <feather-icon
                          icon="CpuIcon"
                          class="mr-50"
          ...
          <br>
          <div
            v-if="baseData.contractsSentToCustomerBackendUserSendTo !== '' && baseData.contractsSentToCustomerBackendUserSendTo !== null"
          >
            <b-alert
              variant="primary"
              show
            >
              <div class="alert-body">
                <strong>solar.family Bearbeiter (Kalendereintrag):</strong> {{ baseData.contractsSentToCustomerBackendUserSendTo }}
              </div>
            </b-alert>
          </div>
          <validation-observer
            v-if="baseData.orderInterest === true && baseData.contractsSentToCustomer !== true && baseData.contractsReviewed === true"
            ref="validateContractsSentToCustomerBackend"
          >
            <b-form
              @submit.prevent="updateContractSentStatus"
            >
              <b-form-group
                label="solar.family Bearbeiter (Kalendereintrag senden)"
                label-for="contractsSentToCustomerBackendUserSendTo"
              >
                <validation-provider
                  #default="{ errors }"
                  name="Email-Adresse"
                ...
                >
                  <b-form-input
                    id="contractsSentToCustomerBackendUserSendTo"
                    v-model="baseData.contractsSentToCustomerBackendUserSendTo"
                    trim
              ...
              <b-button
                ...
                block
                :disabled="isDisabled()"
                @click.prevent="updateContractSentStatus"
              >
                <feather-icon
                ...
                <b-spinner
                  ...
                  small
                />
                Mailversand - Vertragsunterlagen
              </b-button>
            </b-form>
          </validation-observer>
          <b-button
            v-if="baseData.contractsSentToCustomer === true && baseData.contractFilesChecked !== true && baseData.contractsSigned === true"
            size="sm"
            variant="outline-primary"
            class="mb-1 mb-sm-0 mr-0 mr-sm-1"
            type="button"
            block
            :disabled="isDisabled()"
            @click="updateContractFilesChecked()"
          >
            <feather-icon
              v-if="showLoading === false"
              icon="SendIcon"
            ...
            <b-spinner
              ...
              small
            />
            Mailversand - Vertragsunterlagen überprüft
          </b-button>
        </b-col>
      </b-row>
    </b-overlay>
...
import PropertyOwner from './calculation/PropertyOwner.vue'
...
export default {
  ...
  methods: {
    ...
    fileRenameFunction(file) {
      ...
      if (documentType === 21) {
        ...
        fileName = `${fileName}backend_files`
      ...
      } else if (documentType === 201) {
        fileName = `${fileName}sepa`
      }
          ...
          if (value === true) {
            store.dispatch('app-power-plant/deleteFile', { id })
              .then(response => {
                ...
                store.commit('app-power-plant/updateContractsChangedAt')
    ...
    generateEnergySavingContract(plantId, userId, row) {
      store.dispatch('app-power-plant/generateEnergySavingContract', { plantId, userId })
        .then(() => {
          /* eslint-disable no-param-reassign */
          this.$props.fileContainersData.find(x => x.id === row.item.id).rs = 2
          /* eslint-enable no-console */
    ...
    generateContractBillingSheet(plantId, userId, row) {
      store.dispatch('app-power-plant/generateContractBillingSheet', { plantId, userId })
        .then(() => {
          /* eslint-disable no-param-reassign */
          this.$props.fileContainersData.find(x => x.id === row.item.id).rs = 2
          /* eslint-enable no-console */
    ...
    generateMandateCompletion(plantId, row) {
      store.dispatch('app-power-plant/generateMandateCompletion', { plantId })
        .then(() => {
          /* eslint-disable no-param-reassign */
          this.$props.fileContainersData.find(x => x.id === row.item.id).rs = 2
          /* eslint-enable no-console */
    ...
    generateMandateBilling(plantId, row) {
      store.dispatch('app-power-plant/generateMandateBilling', { plantId })
        .then(() => {
          /* eslint-disable no-param-reassign */
          this.$props.fileContainersData.find(x => x.id === row.item.id).rs = 2
          /* eslint-enable no-console */
    ...
    generateMandateBillingNet(plantId, row) {
      store.dispatch('app-power-plant/generateMandateBillingNet', { plantId })
        .then(() => {
          /* eslint-disable no-param-reassign */
          this.$props.fileContainersData.find(x => x.id === row.item.id).rs = 2
          /* eslint-enable no-console */
    ...
    */
          ...
          this.$bvModal
            ...
            .then(value => {
              if (value === true) {
                ...
                store.dispatch('app-power-plant/updateWorkflowStatusEventNotify', { plantId: this.$props.baseData.id, status: 'contractsSentToCustomer', email: this.$props.baseData.contractsSentToCustomerBackendUserSendTo })
                  .then(() => {
    ...
    updateContractFilesChecked() {
      this.$bvModal
        ...
        .then(value => {
          if (value === true) {
            this.showLoading = true
            store.dispatch('app-power-plant/updateWorkflowStatus', { plantId: this.$props.baseData.id, status: 'contractFilesChecked' })
              .then(() => {
                store.commit('app-power-plant/updateSolarPlantUpdatedAt')
                this.$toast({
                  component: ToastificationContent,
    ...
    updateContractsReviewedStatus() {
      console.log('aaa')
      store.dispatch('app-power-plant/simpleStatusUpdate', { id: this.$props.baseData.id, status: 'contractsReviewed', value: this.$props.baseData.contractsReviewed })
        .then(() => {
          store.commit('app-power-plant/updateSolarPlantUpdatedAt')
          this.$toast({
            component: ToastificationContent,
    ...
    updateContractsSignedStatus() {
      store.dispatch('app-power-plant/simpleStatusUpdate', { id: this.$props.baseData.id, status: 'contractsSigned', value: this.$props.baseData.contractsSigned })
        .then(() => {
          store.commit('app-power-plant/updateSolarPlantUpdatedAt')
          this.$toast({
            component: ToastificationContent,
    ...
    propertyOwnerListFinished() {
      store.dispatch('app-power-plant/simpleStatusUpdate', { id: this.$props.baseData.id, status: 'propertyOwnerListFinished', value: this.$props.baseData.propertyOwnerListFinished })
        .then(() => {
          store.commit('app-power-plant/updateSolarPlantUpdatedAt')
          this.$toast({
            component: ToastificationContent,
  ...
  setup(props) {
    ...
    const fetchPropertyOwners = () => {
      store.dispatch('app-power-plant/getPropertyOwnerList', { solarPlantId: router.currentRoute.params.id })
    ...
    const propertyOwnerFetchedAd = computed(() => store.getters['app-power-plant/propertyOwnerFetchedAd'])
    watch(propertyOwnerFetchedAd, (val, oldVal) => {
      if (val > oldVal) {
        fetchPropertyOwners()
      }
    })
    ...
    const generateContractFiles = () => {
      store.dispatch('app-power-plant/generateEnergySavingContract', { plantId: props.baseData.id, userId: props.User.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
          console.log('generateEnergySavingContract generated')
        })

      store.dispatch('app-power-plant/generateContractBillingSheet', { plantId: props.baseData.id, userId: props.User.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
          console.log('generateContractBillingSheet generated')
        })

      store.dispatch('app-power-plant/generateMandateCompletion', { plantId: props.baseData.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
          console.log('generateMandateCompletion generated')
        })

      store.dispatch('app-power-plant/generateMandateBilling', { plantId: props.baseData.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
          console.log('generateMandateBilling generated')
        })

      store.dispatch('app-power-plant/generateMandateBillingNet', { plantId: props.baseData.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
          console.log('generateMandateBillingNet generated')
        })

      store.dispatch('app-power-plant/generateSepa', { plantId: props.baseData.id })
        .then(() => {
          //  PlantCalculationAndLetterContainers.value[1].rs = 2
      ...
      setTimeout(() => {
        //  fetchContainers()
        store.commit('app-power-plant/updateContractsGeneratedAt')
        store.commit('app-power-plant/updateContractsChangedAt')
    ...
    const updateOrderInterest = () => {
      store.dispatch('app-power-plant/updateWorkflowStatus', { plantId: props.baseData.id, status: 'orderInterest' })
        .then(() => {
    ...
    const checkUploadedDocuments = () => {
      store.dispatch('app-power-plant/checkUploadedDocuments', { id: router.currentRoute.params.id, type: 'contract' })
        .then(res => {
    ...
    const contractsChangedAt = computed(() => store.getters['app-power-plant/contractsChangedAt'])
    watch(contractsChangedAt, (val, oldVal) => {
      if (val > oldVal) {
        checkUploadedDocuments()
      }
    })
