<template>
  <div>
    <b-row>
      ...
      >
        <b-form-checkbox
          v-if="segmentStatus > 0"
          id="planDocumentUploaded"
          v-model="baseData.planDocumentUploaded"
          style="float:right;"
          switch
          inline
          :disabled="isDisabled()"
          @change="updatePlanStatus"
        >
          Uploads geprüft
        </b-form-checkbox>
        <b-form-checkbox
          v-if="segmentStatus < 1"
          style="float:right;"
          switch
          inline
          :disabled="true"
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <a
                ...
                v-b-modal.modal-previewPDFPlan
                nohref
              ...
              <a
                v-if="data.item.fileContentType !== 'application/pdf'"
                v-b-modal.modal-previewImagePlan
                nohref
                @click="loadPreview(data.item.id)"
              >
                {{ data.item.fileName }}
          ...
          <br>
          <b-button
            v-b-modal.modal-uploadSinglePlan
            size="sm"
...
export default {
  ...
  methods: {
    ...
    fileRenameFunction(file) {
      //  let fileName = `${this.$props.user.firstName.trim().toLowerCase()}_${this.$props.user.lastName.trim().toLowerCase()}_planungsdokument`
      let fileName = `${this.$props.user.firstName.toLowerCase()}_${this.$props.user.lastName.toLowerCase()}_planungsdokument`
    ...
    updatePlanStatus() {
      store.dispatch('app-power-plant/simpleStatusUpdate', { id: this.$props.baseData.id, status: 'planDocumentUploaded', value: this.$props.baseData.planDocumentUploaded })
        .then(() => {
          store.commit('app-power-plant/updateSolarPlantUpdatedAt')
          this.$toast({
  ...
  setup(props) {
    ...
    const getContainerByTypeId = () => {
      const container = props.fileContainersData.filter(c => (c.type === 22))[0]
