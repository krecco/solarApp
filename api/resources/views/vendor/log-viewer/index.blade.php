...
<script>
    ...
    // Add Bearer token from query parameter or current auth
    @if(request('token'))
        window.LogViewer.headers['Authorization'] = 'Bearer {{ request('token') }}';
    @elseif(auth()->check() && auth()->user()->currentAccessToken())
        window.LogViewer.headers['Authorization'] = 'Bearer {{ auth()->user()->currentAccessToken()->plainTextToken }}';
    @endif
</script>
