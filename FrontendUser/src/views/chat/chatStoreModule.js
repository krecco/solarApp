...
export default {
  ...
  actions: {
    ...
    getMessagesForUser(ctx, { userId }) {
      return new Promise((resolve, reject) => {
        axios
          .get(`/message/get-messages/${userId}`)
          .then(response => resolve(response))
          .catch(error => reject(error))
      })
    },

    getMessagesForFrontendUser(ctx, { userId, currentChatPage, records }) {
      return new Promise((resolve, reject) => {
        axios
          .get(`/message/get-messages-frontend/${userId}/${currentChatPage}/${records}`)
          .then(response => resolve(response))
          .catch(error => reject(error))
      })
    },
